...
if (!token.startsWith('xoxp-')) {
  console.error(
    `Expected a user token (xoxp-), got "${token.slice(0, 5)}…". RTS needs a user token with ${REQUIRED_SCOPE}.`,
  );
  process.exit(2);
}
  ...
  console.error(
    `\n❌ RTS call failed: ${result.error}` +
      (result.error === 'missing_scope'
        ? ` (need ${REQUIRED_SCOPE})`
        : result.error === 'not_allowed_token_type'
          ? ' (RTS needs a user token from a directory-published or internal app)'
          : ''),
  );
  process.exit(1);
}
